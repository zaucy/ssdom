...
class ServerSideDOM {
  ...
  callExtensionFunction() {
    ...
    for(let i=0; this._extensions.length > i; i++) {
      ...
      let extensionModuleName = `ssdom-ext-${extensionName}`;
      var extensionModulePath = process.cwd() + "/node_modules/" + extensionModuleName;
      let extensionModule = require(extensionModulePath);
