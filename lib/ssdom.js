...
class ServerSideDOMRequestHandler {
  ...
  fullContentPathname(pathname) {
    var fullPathname = this.privateDir + "/content" + pathname;
  ...
  loadContent(pathname, cloned) {
    ...
    if(typeof this._cachedContent[pathname] == "undefined") {
      let data = fs.readFileSync(fullPathname);

      var wrapEl = this.document.createElement("div");
  ...
  loadData(pathname, copied) {
    ...
    if(typeof this._cachedData[pathname] == "undefined") {
      let data = fs.readFileSync(this.privateDir + "/data" + pathname);
  ...
  loadScriptSource(pathname) {
    ...
    var data = "";
    try {
      data = fs.readFileSync(this.privateDir + "/scripts" + pathname);
    } catch(err) {
      try { data = fs.readFileSync(this.publicDir + pathname); } catch(err) { }
    }

    return "const SERVER=true;" + data;
  }
  ...
  loadResource(resource, callback) {
    ...
    callback(null, this.loadScriptSource(pathname));
  }
